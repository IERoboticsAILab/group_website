...
class HomeContent(models.Model):
    ...
    about_markdown_content = models.TextField(
        blank=True, help_text="Markdown content for the home page main section"
    )
    youtube_video_url = models.URLField(
        blank=True, help_text="YouTube video embed URL for the home page"
    )
    section_markdown_content = models.TextField(
        blank=True, help_text="Markdown content for the second home page section"
    )
    section_image = FilerImageField(
        null=True,
        blank=True,
...
class ResearchLine(models.Model):
    ...
    content = RichTextField(
        blank=True, help_text="Rich text content with images for the research line"
    )
    ...
    team_members = models.ManyToManyField(
        "TeamMember", blank=True, related_name="research_lines"
    )
    publications = models.ManyToManyField(
        "Publication", blank=True, related_name="research_lines"
    )
    projects = models.ManyToManyField(
        "ResearchProject", blank=True, related_name="research_lines_set"
    )
...
class ResearchLineGalleryImage(models.Model):
    research_line = models.ForeignKey(
        ResearchLine, on_delete=models.CASCADE, related_name="gallery_images"
    )
...
class ResearchProject(models.Model):
    ...
    team_members = models.ManyToManyField(
        "TeamMember", blank=True, related_name="projects"
    )
    publications = models.ManyToManyField(
        "Publication", blank=True, related_name="projects"
    )
    slug = models.SlugField(max_length=250, unique=True, blank=True)
    active = models.BooleanField(default=True)
...
class ProjectGalleryImage(models.Model):
    project = models.ForeignKey(
        ResearchProject, on_delete=models.CASCADE, related_name="gallery_images"
    )
    image = FilerImageField(
        null=True, blank=True, related_name="project_gallery", on_delete=models.SET_NULL
    )
    caption = models.CharField(max_length=200, blank=True)
    order = models.PositiveIntegerField(default=0)
...
class TeamMember(models.Model):
    ...
    image = FilerImageField(
        null=True, blank=True, related_name="member_images", on_delete=models.SET_NULL
    )
...
class Publication(models.Model):
    ...
    websitelink = models.URLField(blank=True)
